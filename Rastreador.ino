...
// Configurações da rede móvel e servidor
String APN = ""; 
String SERVER_URL = "";  
...
bool sendHTTPPost(float lat, float lng){
  String response;
  String data = "{\"latitude\":" + String(lat,6) + ",\"longitude\":" + String(lng,6) + "}";
  ...
  sendToA9G("AT+HTTPACTION=1", &response, 6000);
...
void loop(){  
  ...
  if(millis() - prevSendMillis > intervalSend && gps.location.isValid()){
    ...
    if(sendHTTPPost(gps.location.lat(), gps.location.lng())){
      printMsg("Localizacao enviada!", true);
    }else{
      printMsg("Erro ao enviar!", true);
    }
  }
}
